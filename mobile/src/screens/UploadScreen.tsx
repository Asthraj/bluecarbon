...
type QueueItem = { id: string; uri: string; lat?: number; lon?: number; notes?: string };
...
export default function UploadScreen() {
  ...
  const addSample = async () => {
    ...
    await FileSystem.writeAsStringAsync(sampleUri, 'SAMPLE', { encoding: FileSystem.EncodingType.UTF8 });
    const item: QueueItem = { id: Date.now().toString(), uri: sampleUri, lat: location?.coords.latitude, lon: location?.coords.longitude, notes };
  ...
  const uploadItem = async (item: QueueItem) => {
    try {
      ...
      form.append('file', { uri: item.uri, name: item.id + '.jpg', type: 'image/jpeg' } as any);
      form.append('lat', String(item.lat ?? ''));
      form.append('lon', String(item.lon ?? ''));
      form.append('notes', item.notes ?? '');

      await fetch('https://example.com/api/upload', { method: 'POST', body: form as any });
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.muted}>Latitude: {location?.coords.latitude?.toFixed(4) ?? '—'} Longitude: {location?.coords.longitude?.toFixed(4) ?? '—'}</Text>
      ...
      <FlatList
        ...
        keyExtractor={(i) => i.id}
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => setSelected(item.id)} style={styles.queueItem}>
            <View>
...
const styles = StyleSheet.create({
  ...
  input: { borderWidth: 1, borderColor: '#ddd', padding: 10, borderRadius: 8, marginTop: 12 },
  queueItem: { padding: 12, borderRadius: 8, borderWidth: 1, borderColor: '#eee', marginTop: 8, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }
});
